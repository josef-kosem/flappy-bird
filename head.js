...
CloudflareApps.siteId = "b1f8539628d9b7197ebd271dc8969a87";
...
CloudflareApps.installs["3LcBDNPrmWj9"] = {
    appId: "ynYCifFKSA_0",
    scope: {}
};;
CloudflareApps.installs["3LcBDNPrmWj9"].options = {
    "PPLanguage": "en",
    "addCPBanner": false,
    "addPPBadge": false,
    "addPPBanner": false,
...
CloudflareApps.installs["3LcBDNPrmWj9"].product = {
    "id": "iubenda-legacy"
};;
if (CloudflareApps.matchPage(CloudflareApps.installs['3LcBDNPrmWj9'].URLPatterns)) {
    (function () {
        'use strict'
        window._iub = window._iub || []
        var options = CloudflareApps.installs['3LcBDNPrmWj9'].options
        var isPreview = "3LcBDNPrmWj9" === 'preview'
        var PPLanguage = options.PPLanguage
        var ppPublicId = options.ppPublicId
        var siteId = options.siteId
        var previewMockupIds = {
            en: {
                'ppPublicId': 8106822,
                'siteId': 798710
            },
        ...
        function documentHasPrivacyPPBadge() {
            if (document.getElementById('iubenda-ibadge')) return true
        ...
        function addCPBannerScript() {
            window._iub.csConfiguration = {
                cookiePolicyId: ppPublicId,
                siteId: siteId,
                lang: PPLanguage
            }
            var vendorScript = document.createElement('script')
        ...
        function bootstrap() {
            if (isPreview) {
                ppPublicId = previewMockupIds[PPLanguage]['ppPublicId']
                siteId = previewMockupIds[PPLanguage]['siteId']
            }
            var addPPBadge
            if (typeof options.addPPBadge !== 'undefined') {
                addPPBadge = options.addPPBadge
            } else if (typeof options.addPPBanner !== 'undefined') {
                addPPBadge = options.addPPBanner
            } else if (typeof options.skip_badge !== 'undefined') {
                addPPBadge = !options.skip_badge
            }
            if (addPPBadge && ppPublicId && !documentHasPrivacyPPBadge()) {
                addPrivacyPolicy()
            }
            if (options.addCPBanner && siteId) {
                addCPBannerScript()
            }
        }
